...
function puppeteerHelper() {
    return {
        getSelectorText: async (selector: string, callback?: (_result_: string) => unknown) => {
            // screenshot
            await browserInstance.screenshot({ path: 'example.png' });
        ...
        getAllBids: async (callback?: (_result_: Bid[]) => unknown) => {
            ...
            const _result_ = await browserInstance.evaluate(() => {
                var bids: Bid[] = [];
                var bidElements = document.querySelectorAll('.table-fixed > tbody > tr');
                bidElements.forEach((element: HTMLElement) => {
                    var bidder = (element.children[0] as HTMLElement).innerText;
                    var bid = (element.children[1] as HTMLElement).innerText;
                    var time = (element.children[3] as HTMLElement).innerText;
        ...
        async getAllImages() {
            const _result_ = await browserInstance.evaluate(() => {
                var images: string[] = [];
                var imageElements = document.querySelectorAll('.image-gallery-item__image');
